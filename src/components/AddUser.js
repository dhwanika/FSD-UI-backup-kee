...
if(this.state.sort){
  ...
  {return data.filter(fNameFilter).map((user) =>
      ...
      <div className="col-md-3 displayInlineFlex">
         ...
         <button type="button" className="btn btn-primary" onClick={() =>{this.deleteUser(user)}}>Delete</button>
      </div>
      </div>
      </div>
    )}
...
<React.Fragment>
    ...
    <div className="col-md-1">
      <button type="button" class="btn btn-primary" onClick={()=>{this.updateUser()}}>Update</button>
    </div> :
    <div className="col-md-1">
